...
// satisfies runutil.Worker via Run, so it can be wrapped in a
// runutil.DeclarativeWorker and registered with runutil.ProvideWorker:
//
//	b := chutil.New[Row](conn, insertSQL, 2000, 2*time.Second)
//	runutil.DeclarativeWorker{Name: "ClickHouse", Worker: b}
//
...
// New creates a Batcher. It takes an already-opened connection so the consuming
// project owns the connection configuration (address, auth, compression). The
// insertSQL must be a column-qualified INSERT statement, e.g.
// `INSERT INTO db.table (col_a, col_b)`.
func New[T any](conn Conn, insertSQL string, maxSize int, maxWait time.Duration) *Batcher[T] {
	return &Batcher[T]{
		conn:        conn,
		insertSQL:   insertSQL,
		maxSize:     maxSize,
